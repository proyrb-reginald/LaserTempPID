...
rt_err_t rt_get_errno(void)
{
    ...
    tid = rt_thread_self();
    if (tid == RT_NULL) return __rt_errno;
...
int *_rt_errno(void)
{
    ...
    if (rt_interrupt_get_nest() != 0) return (int *)&__rt_errno;
...
#define TOO_SMALL(LEN) ((LEN) < LBLOCKSIZE)
    ...
    if (!TOO_SMALL(count) && !UNALIGNED(s)) {
        /* If we get this far, we know that count is large and s is
         * word-aligned. */
        ...
        if (LBLOCKSIZE == 4) {
            ...
            buffer = 0;
            for (i = 0; i < LBLOCKSIZE; i++) buffer = (buffer << 8) | d;
        }
...
RT_WEAK void *rt_memcpy(void *dst, const void *src, rt_ubase_t count)
{
    ...
    if (tmp <= s || tmp > (s + count)) {
        ...
        for (len = count; len > 0; len--) tmp[len - 1] = s[len - 1];
    }
...
char *rt_strstr(const char *s1, const char *s2)
{
    ...
    while (l1 >= l2) {
        l1--;
        if (!rt_memcmp(s1, s2, l2)) return (char *)s1;
        s1++;
    }
...
void rt_show_version(void)
{
    ...
    rt_kprintf(" / | \\  2006-2022 Copyright by RT-Thread\n");
}
...
rt_inline void _heap_unlock(rt_base_t level)
{
    ...
    if (rt_thread_self()) rt_mutex_release(&_lock);
#else
    rt_exit_critical();
#endif
}
...
rt_inline void _smem_info(rt_size_t *total, rt_size_t *used,
                          rt_size_t *max_used)
{
    if (total) *total = system_heap->total;
    if (used) *used = system_heap->used;
    if (max_used) *max_used = system_heap->max;
}
...
int __rt_ffs(int value)
{
    ...
    if (value & 0xff) return __lowest_bit_bitmap[value & 0xff] + 1;

    if (value & 0xff00) return __lowest_bit_bitmap[(value & 0xff00) >> 8] + 9;
